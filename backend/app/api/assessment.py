...
from app.models import Assessment, Client, ProcessBottleneck, AIUseCase, RiskRegister, RoadmapItem, DocumentSignal, User
...
def create_assessment(
    ...
    if payload.client_id is not None:
        ...
        client = (
            db.query(Client)
            .filter(Client.organization_id == organization.id, Client.name == payload.company_name)
            .first()
        )
        if not client:
            client = Client(
    ...
    ass = Assessment(
        ...
        compliance_requirements=payload.compliance_requirements or (client.compliance_requirements if client else []),
...
def upload_documents(
    ...
    for f in files:
        ...
        try:
            ...
            if f.filename.endswith(".pdf"):
                ...
                paragraph_text = [paragraph.text for paragraph in doc.paragraphs if paragraph.text.strip()]
                table_text = []
            ...
            signals_to_create.append(DocumentSignal(
    ...
    try:
        ...
        # Risk assessment
        ass.risk_level = "High" if any(r.get("severity") == "High" for r in final_state.get("risks", [])) else "Medium"
        ...
        db.commit()
        raise HTTPException(status_code=500, detail=f"AI Graph analysis failed: {str(e)}")
...
def update_assessment(
    ...
    for key, value in update_data.items():
        if key == "client_id":
            ...
            continue
        if key == "approval_status" and value is not None and value not in VALID_APPROVAL_STATUSES:
    ...
    if bottlenecks is not None:
        db.query(ProcessBottleneck).filter(ProcessBottleneck.assessment_id == id).delete()
...
def export_assessment_report(
    ...
    if doc_format.lower() == "pdf":
        ...
        return StreamingResponse(
            ...
            headers={"Content-Disposition": f"attachment; filename={ass.company_name.replace(' ', '_')}_AI_Readiness_Report.pdf"}
        )
        ...
        return StreamingResponse(
            ...
            headers={"Content-Disposition": f"attachment; filename={ass.company_name.replace(' ', '_')}_AI_Pilot_Proposal.docx"}
        )
        ...
        return StreamingResponse(
            ...
            headers={"Content-Disposition": f"attachment; filename={ass.company_name.replace(' ', '_')}_Board_Presentation.pptx"}
        )
    else:
...
def create_walkthrough_workspace(
    ...
    ass = Assessment(
        ...
        industry="Professional Services",
        company_size="100-500 employees",
        departments=["Operations", "Customer Support", "Sales & Pre-sales", "Compliance & Governance"],
        current_tools=["Salesforce", "Microsoft Excel", "Jira Service Desk", "SharePoint"],
        cloud_preference="Azure",
        compliance_requirements=["GDPR", "SOC2 Type II"],
        ...
        pain_points=["Manual Process Overload", "Data Silos", "Slow Support Response Times"],
