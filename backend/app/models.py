...
class Assessment(Base):
    ...
    confidence_score = Column(Float, default=0.0) # confidence on recommendation (e.g. 85%)
    status = Column(String, default="intake") # intake, uploading, processing, completed
    ...
    bottlenecks = relationship("ProcessBottleneck", back_populates="assessment", cascade="all, delete-orphan")
    use_cases = relationship("AIUseCase", back_populates="assessment", cascade="all, delete-orphan")
    risks = relationship("RiskRegister", back_populates="assessment", cascade="all, delete-orphan")
    roadmap_items = relationship("RoadmapItem", back_populates="assessment", cascade="all, delete-orphan")
    extracted_signals = relationship("DocumentSignal", back_populates="assessment", cascade="all, delete-orphan")
...
class AIUseCase(Base):
    ...
    evidence = Column(Text, nullable=True) # Recommended because uploaded documents show...
...
class DocumentSignal(Base):
    ...
    signal_type = Column(String, nullable=False) # SOP, Process, Tech Stack, Governance Gap
    description = Column(Text, nullable=False)
