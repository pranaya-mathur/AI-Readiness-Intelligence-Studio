...
class DocumentGenerators:
    ...
    def cleanup_export_data(assessment: Dict[str, Any]) -> Dict[str, Any]:
        ...
        test_keywords = ["e2e test", "overridden", "test override", "programmatically", "validator", "strict sales and support log parsing audits"]
        ...
        has_markers = (
            contains_test_marker(client_sum) or 
            contains_test_marker(rec_pilot) or 
            contains_test_marker(why_pilot) or 
            contains_test_marker(exp_impact) or
            contains_test_marker(rev_notes)
        )
        ...
        if "use_cases" in data:
            for uc in data["use_cases"]:
                uc["department"] = local_normalize(uc.get("use_case_name"), uc.get("description"))
                
        # 3. Sharpen Risk Register Output (Issue 3)
        if "risks" in data:
            for r in data["risks"]:
                risk_name = (r.get("risk_name") or "").lower()
                if "sensitive data" in risk_name or "exposure" in risk_name or "leak" in risk_name:
                    r["recommendation"] = "Enforce human approval, source logging, redaction controls, and access restrictions before AI-generated proposal or support outputs are shared externally."
                elif "low-trust" in risk_name or "un-grounded" in risk_name or "evidence" in risk_name:
                    r["recommendation"] = "Require reviewer validation and evaluation baselines before AI outputs are used in client-facing or operational workflows."
                else:
    ...
    def _summary_text(assessment: dict) -> str:
        ...
        return sanitize_text(summary, "A comprehensive audit mapping corporate structures, manual workflows, and high-impact automated opportunities.")
    ...
    def get_client_facing_evidence(source_file: str, original_description: str) -> str:
        ...
        if "successfully extracted text" in desc.lower() or "all 9 nodes" in desc.lower() or len(desc) < 15:
        ...
        return sanitize_text(original_description, "Confirmed parsed client document insights.")

    @staticmethod
    def get_clean_roadmap_item(phase: str, original_action: str, original_impact: str) -> tuple[str, str]:
        """
        ...
        if "30-Day" in p:
            ...
            impact = sanitize_text(original_impact, "Reduces turnaround times and manual efforts.")
    ...
    def calculate_commercial_strategy(assessment: Dict[str, Any]) -> Dict[str, Any]:
        ...
        avg_score = int(round((d_score + p_score + i_score + g_score + s_score + t_score + b_score) / 7.0))
        
        recommended_pilot = assessment.get("recommended_first_pilot") or "Intelligent Pre-Sales Proposal Copilot"
        ...
        if model == "Managed AI Governance Retainer":
            pricing_range = f"INR {format_lakh(base_min)} – INR {format_lakh(base_max)} per month"
        else:
        ...
        if is_apex:
            justifications = [
                {"title": "Sales & Pre-sales", "desc": "Intelligent Pre-Sales Proposal Copilot requires RAG semantic parsing over proprietary playbooks."},
                {"title": "Customer Support", "desc": "Autonomous Support Triage Router demands layout classification and log parsing."},
                ...
                {"title": "Human Review Workflow", "desc": "Draft-to-Approved validation screens to prevent operational model hallucinations."}
            ]
        else:
            justifications = []
            if dept_count > 0:
    ...
    def generate_pdf_report(assessment: Dict[str, Any]) -> io.BytesIO:
        ...
        is_real_doc = any(
            s.get("source_file") and 
            s.get("source_file").endswith((".docx", ".pdf", ".txt")) and 
            not s.get("source_file").endswith("_brief.txt")
            for s in signals
        )
        grounding_mode = "Document-Grounded Evidence" if is_real_doc else "Structured Brief Evidence"
        ...
        <body>
                ...
                <table class="grid-2">
                    <tr>
                        <td class="grid-cell" style="width: 35%; text-align: center;">
                            ...
                            <p style="font-size: 9pt; color: #475569;">Confidence level: {assessment.get('confidence_score', 85.0)}%</p>
                        </td>
                        <td class="grid-cell" style="width: 65%;">
                            <div style="font-weight: bold; color: #64748b; margin-bottom: 4px;">Transformation Impact</div>
                            <p style="margin: 0 0 8px 0; font-size: 10pt;"><strong>Estimated Automation Potential:</strong> {int(assessment.get('automation_potential', 28))}%</p>
                            <p style="margin: 0 0 4px 0; font-size: 10pt;"><strong>Recommended First Pilot:</strong> {sanitize_text(assessment.get('recommended_first_pilot'), "Intelligent Pre-Sales Proposal Copilot")}</p>
                            <p style="margin: 0; font-size: 9pt; color: #475569; line-height: 1.4;">{sanitize_text(assessment.get('why_recommended_pilot'), "High transformation value, low complexity, and direct sales impact.")}</p>
                        </td>
                    </tr>
                </table>
            </div>
            ...
            <div class="section">
                ...
                <p style="font-size: 9pt; color: #475569; margin-top: 10px; line-height: 1.4;"><strong>Interpretation:</strong> {sanitize_text(assessment.get('readiness_interpretation'), "The client presents a standard readiness path.")}</p>
            </div>
            ...
            <div class="section" style="page-break-before: always;">
                ...
                <table class="grid-2">
                    <tr>
                        <td class="grid-cell" style="width: 50%;">
                            ...
                            <div style="font-size: 11pt; font-weight: bold; color: #0f172a;">{comm.get("model")}</div>
                            <p style="margin-top: 6px; font-size: 8.5pt; color: #475569; line-height: 1.4;"><strong>Focus:</strong> {comm.get("rationale")}</p>
                        </td>
                        <td class="grid-cell" style="width: 50%;">
                            <div style="font-weight: bold; color: #3b82f6; font-size: 9.5pt; margin-bottom: 4px;">Indicative Investment</div>
                            <div style="font-size: 12pt; font-weight: 800; color: #10b981;">{comm.get("pricing_range")}</div>
                ...
                <div style="margin-top: 15px;">
                    ...
                    <ul style="margin: 0; padding-left: 20px; font-size: 9pt; color: #334155;">
                        {"".join(f"<li style='margin-bottom:4px;'><strong>{just.get('title')}:</strong> {just.get('desc')}</li>" for just in comm.get("justifications", []))}
                    </ul>
                </div>
    ...
    def generate_docx_proposal(assessment: Dict[str, Any]) -> io.BytesIO:
        ...
        p.add_run(f"{sanitize_text(assessment.get('recommended_first_pilot'), 'Intelligent Pre-Sales Proposal Copilot')}\n")
        p.add_run("Why recommended: ").bold = True
        p.add_run(f"{sanitize_text(assessment.get('why_recommended_pilot'), 'High transformation value, low complexity, and direct sales impact.')}\n")
        p.add_run("Expected Impact: ").bold = True
        p.add_run(f"{sanitize_text(assessment.get('expected_pilot_impact'), 'Reduces proposal creation cycles and improves approved content reuse.')}")
        ...
        for u in assessment.get("use_cases", []):
            ...
            row_cells[0].text = f"{sanitize_text(u.get('use_case_name'), 'AI Assistant')}: {sanitize_text(u.get('description'), 'AI decision support.')}"
        ...
        grounding_label = "Document-Grounded Evidence" if is_real_doc else "Structured Brief Evidence"
        ...
        if signals:
            ...
            for s in signals:
                ...
                row_cells[2].text = DocumentGenerators.get_client_facing_evidence(s.get('source_file'), s.get('description', ''))
        ...
        p_disc.add_run("Disclaimer: Pricing is indicative, assumption-based, and subject to discovery validation.").italic = True
        ...
        for layer in comm.get("architecture_layers", []):
            ...
            row_cells[1].text = f"{layer.get('purpose', '')} ({layer.get('components', '')})"
        ...
        for item in assessment.get("roadmap_items", []):
            ...
            action, impact = DocumentGenerators.get_clean_roadmap_item(item.get("phase"), item.get("action_item"), item.get("expected_impact"))
    ...
    def generate_pptx_deck(assessment: Dict[str, Any]) -> io.BytesIO:
        ...
        txBox = slide.shapes.add_textbox(PtInches(1), PtInches(1.8), PtInches(8), PtInches(4.5))
        tf = txBox.text_frame
        ...
        if signals:
            ...
            table_shape = slide.shapes.add_table(rows, 4, PtInches(0.5), PtInches(1.8), PtInches(9), PtInches(4.2))
            ...
            for idx, s in enumerate(signals[:rows-1]):
                ...
                table.cell(idx+1, 2).text = DocumentGenerators.get_client_facing_evidence(s.get("source_file"), s.get("description", ""))
        ...
        for idx, u in enumerate(use_cases_list[:rows-1]):
            table.cell(idx+1, 0).text = sanitize_text(u.get("use_case_name"), "AI Assistant")
        ...
        txBox_comm = slide.shapes.add_textbox(PtInches(0.8), PtInches(1.8), PtInches(8.4), PtInches(4.5))
        ...
        txBox = slide.shapes.add_textbox(PtInches(1), PtInches(1.8), PtInches(8), PtInches(4.2))
        tf = txBox.text_frame
