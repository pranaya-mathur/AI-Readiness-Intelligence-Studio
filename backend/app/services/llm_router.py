...
class LLMRouter:
    ...
    def generate_completion(
        ...
        if settings.GROQ_API_KEY and not settings.USE_OLLAMA:
            try:
                ...
                if response.status_code == 200:
                    ...
                    logger.warning(f"GROQ failed with code {response.status_code}: {response.text}")
        ...
        model = settings.OLLAMA_STRUCTURED_MODEL if require_json else settings.OLLAMA_REASONING_MODEL
        try:
            ...
            if response.status_code == 200:
                ...
                logger.warning(f"Ollama failed with code {response.status_code}: {response.text}")
        except Exception as e:
            logger.warning(f"Error connecting to Ollama: {e}")
            
        # 3. Last resort: Mock response generator based on prompts
        logger.warning("Both Groq and Ollama are unavailable. Routing to Local Deterministic Fallback Engine.")
    ...
    def generate_embeddings(text: str) -> List[float]:
        ...
        try:
            ...
            logger.warning(f"Ollama embeddings failed ({e}). Generating static placeholder floats.")
    ...
    def _generate_fallback_response(prompt: str, require_json: bool) -> str:
        ...
        compliance = _extract_list(prompt, "Compliance Requirements") or _extract_list(prompt, "For these compliance requirements")
        ...
        def bottleneck_from_pain(pain: str, idx: int) -> Dict[str, Any]:
            ...
            if "proposal" in lower_pain or "sales" in lower_pain:
                return {
                    "department": "Sales & Pre-sales" if "Sales & Pre-sales" in mapped_departments else department,
            ...
            if "support" in lower_pain or "ticket" in lower_pain or "customer" in lower_pain:
                return {
                    "department": "Customer Support" if "Customer Support" in mapped_departments else department,
            ...
            if "compliance" in lower_pain or "audit" in lower_pain:
                return {
                    "department": "Compliance & Governance" if "Compliance & Governance" in mapped_departments else department,
        ...
        def use_case_from_bottleneck(bottleneck: Dict[str, Any], idx: int) -> Dict[str, Any]:
        ...
        derived_bottlenecks = [bottleneck_from_pain(pain, idx) for idx, pain in enumerate(pain_points[:3])]
        derived_use_cases = [use_case_from_bottleneck(item, idx) for idx, item in enumerate(derived_bottlenecks[:3])]
        ...
        if "support" in goals_lower or "ticket" in goals_lower:
            derived_use_cases.insert(1 if derived_use_cases else 0, {
                "use_case_name": "Support Intake and Draft Response Assistant",
                "department": "Customer Support" if "Customer Support" in departments else departments[0],
        ...
        if require_json:
            if "use_case" in prompt.lower() or "opportunity" in prompt.lower():
                ...
                top_use_case = derived_use_cases[0]["use_case_name"] if derived_use_cases else "AI workflow assistant"
                return json.dumps({
                    "roadmap": [
                        {
                            "phase": "30-Day",
            ...
            primary_pains = ", ".join(pain_points[:2]).lower() or "manual coordination work"
